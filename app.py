...
# 2. 영상 생성 (6:4)
    ...
    st.subheader("🎬 6:4 시나리오 자동 생성")
    
    if not st.session_state["scenes"]:
        st.info("먼저 '시나리오 입력' 메뉴에서 장면을 추가해 주세요.")
    else:
        for scene in st.session_state["scenes"]:
            st.markdown(f"### 🎬 Scene {scene['scene_num']}")
            st.write(f"- **상황:** {scene['situation']}")
            
            # AI 프롬프트 텍스트 블록 생성
            video_audio_prompt = f"""[마스터 고정값: Seedance 2.0 캐릭터 일관성 유지]
- (6초: 입술 은폐 구도 및 안면 연기, 시네마틱 톤)
- [오디오: 한국군 사령관의 목소리로 '{scene['dialogue']}' 비장하게 발화]
- (대사 리듬에 맞춘 격렬한 하얀 입김 묘사)
- (4초: {scene['asset']}의 하이퍼 리얼리즘 액션)
- [배경음: 엔진음 및 폭발음]"""
            
            # 사용자가 쉽게 복사할 수 있도록 text_area로 출력
            st.text_area(
                label=f"Scene {scene['scene_num']} AI 비디오/오디오 프롬프트", 
                value=video_audio_prompt, 
                height=180,
                key=f"prompt_{scene['scene_num']}"
            )
            st.markdown("---")

# 3. 이미지 생성
elif menu == "이미지 생성 (한국군 사령관)":
    st.subheader("🎨 한국군 사령관 실사화 프롬프트")
